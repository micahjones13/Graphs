...
# print(roomGraph[0][1])# ---> Gives the directions. [0] is the id, [0][0] is the coodrinates of that id, [0][1] is the directions
# print(roomGraph[0][1]['n']) --> gives the roomID to the north
...
# print(visited)
# start = rooms[player.currentRoom.id] = player.currentRoom.getExits()
...
#     print(path, 'path')
#     # print(r, 'R')
#     # print(r.getRoomInDirection('n')) # --> returns the room obj to the 'n'
#     # print(r.getExits()) # ---> gives a list of avalibe exits ['n', 's', 'w', 'e']
...
#                 # ---> gives the string of each direction. ie 'n'
#             print(exit, 'EXIT')
#             # print(r.getRoomInDirection(exit), 'GET ROOM') --> gives room obj in direction of exit
#             # need to figures out a way to append each direction moved into the array.
#             # I'm hitting every room, but the list isn't complete for some reason.
#             # I need to add the direction it's going to get the next neighbor.
#             # next_room = r.getRoomInDirection(exit)
#             # north_room = r.getRoomInDirection('n')
#             # south_room = r.getRoomInDirection('s')
#             # east_room = r.getRoomInDirection('e')
#             # west_room = r.getRoomInDirection('w')

#             # if next_room is not None:
#             #     if north_room is not None and next_room.id == north_room.id:
#             #         traversalPath.append('n')

#             #     if south_room is not None and next_room.id == south_room.id:
#             #         traversalPath.append('s')

#             #     if east_room is not None and next_room.id == east_room.id:
#             #         traversalPath.append('e')

#             #     if west_room is not None and next_room.id == west_room.id:
...
#             q.enqueue(path_copy)
#             player.travel(exit)
#             traversalPath.append(player.currentRoom.getExits())
...
# Move player through all the rooms adding the rooms and exits to dicts. When you hit a dead end, backtrack until you can go somewhere else.

# reverse directions 'n' - 's' ect used to back track out of dead ends
reverse_directions = {'n': 's', 's': 'n', 'w': 'e', 'e': 'w'}
# Track all rooms visited in order to compare w/ number of rooms in graph
visited = {}
# Need to be able to get back out of a dead end/we alread travled there -- this stores the last directions moved so that we can move back w/ help of
# reverse directions
back_track = []
# Dictionary for rooms --- this is going to hold dirctions traveled!
rooms = {}


# put in starting room value
# Doing this shoots it down to the travel code at bottom
rooms[player.currentRoom.id] = player.currentRoom.getExits()
visited[player.currentRoom.id] = player.currentRoom.getExits()
# print(visited, 'BEFORE LOOP')
# While not all rooms have been hit...
while len(visited) < len(roomGraph)-1:
    ...
    if player.currentRoom.id not in visited:
        ...
        # print('VISITED')
        # print(visited)
        # add too rooms dict as well
        rooms[player.currentRoom.id] = player.currentRoom.getExits()
        # print('ROOMS')
        # print(rooms)


#  While loop to back up if we've hit a dead end --- the len will be less than one since there will be only 1 dir to go - back
    while len(rooms[player.currentRoom.id]) < 1:
        # we've hit a wall, need to go back. Pop back-track and go that direction
        reverse = back_track.pop()
    ...
    exit_dir = rooms[player.currentRoom.id].pop()

    # print(exit_dir, 'Exit dir')
    #  Add to traversalPath
    traversalPath.append(exit_dir)
    # Add exit to back_track
    back_track.append(reverse_directions[exit_dir])
    # travel exitDirection
    player.travel(exit_dir)
    # print(player.currentRoom.id, 'AFTER TRAVEL')
    # print(visited)
...
# print(world.startingRoom, 'START')
...
"""
this only passed the 9
    traversalPath.append(exit)
            new_room = r.getRoomInDirection(exit)
            if new_room is not None:
                for exits in new_room.getExits():
...
next_room = r.getRoomInDirection(exit)
 ...
 if next_room is not None:
                if north_room is not None and next_room.id == north_room.id:
                    traversalPath.append('n')
                    if r.getRoomInDirection('n') is not None:
                        path_copy = path.copy()

                        path_copy.append(r.getRoomInDirection('n'))
                        q.enqueue(path_copy)

                    # traversalPath.append('s')
                if south_room is not None and next_room.id == south_room.id:
                    traversalPath.append('s')
                    if r.getRoomInDirection('s') is not None:
                        path_copy = path.copy()

                        path_copy.append(r.getRoomInDirection('s'))
                        q.enqueue(path_copy)
                    # traversalPath.append('n')
                if east_room is not None and next_room.id == east_room.id:
                    if r.getRoomInDirection('e') is not None:
                        path_copy = path.copy()

                        path_copy.append(r.getRoomInDirection('e'))
                        q.enqueue(path_copy)
                    # traversalPath.append('e')
                if west_room is not None and next_room.id == west_room.id:
                    if r.getRoomInDirection('w') is not None:
                        path_copy = path.copy()
